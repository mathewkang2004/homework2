...
# Risk rebate data --------------------------------------------------------
# Import data
ma.path.a <- "data/input/ma/cms-payment/2019/2019PartCPlanLevel.xlsx"
...
ma.path.b <- "data/input/ma/cms-payment/2019/2019PartDPlans.xlsx"
...
# FFS data ----------------------------------------------------------------
# Import data
ffs.data <- read_xlsx("data/input/ffs-costs/Extracted Data/FFS2019/FFS19.xlsx",
                      skip=2,
...
ma.2019 <- ma.2019 %>%
  mutate(year = coalesce(as.numeric(year.x), as.numeric(year.y), 2019)) %>%
...
full.2019 <- ma.2019 %>%
  ...
  left_join(ffscosts.2019 %>% select(-state) %>% filter(!is.na(ssa)), by = c("ssa","year")) %>%
  mutate(
