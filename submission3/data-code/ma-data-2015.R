...
plan.data.2015 <- as_tibble(plan.year)
...
service.data.2015 <- service.year %>%
...
# Landscape data ----------------------------------------------------------
# Import data
ma.path.a <- paste0("data/input/ma/landscape/Extracted Data/2015LandscapeSource file MA_AtoM 11042014.csv")
...
ma.path.b <- paste0("data/input/ma/landscape/Extracted Data/2015LandscapeSource file MA_NtoW 11042014.csv")
...
mapd.path.a <- paste0("data/input/ma/landscape/Extracted Data/PartCD/2015/Medicare Part D 2015 Plan Report 03182015.xls")
...
mapd.path.b <- paste0("data/input/ma/landscape/Extracted Data/PartCD/2015/Medicare Part D 2015 Plan Report 03182015.xls")
...
landscape.2015 <- mapd.clean.merge(ma.data=ma.data, mapd.data=mapd.data, y)
...
rebate.2015 <- risk.rebate.a %>%
  left_join(risk.rebate.b, by=c("contractid","planid"))

# FFS data ----------------------------------------------------------------
# Import data
ffs.data <- read_xlsx("data/input/ffs-costs/Extracted Data/Aged Only/FFS15.xlsx",
                     skip=2,
...
# Merge data --------------------------------------------------------------
ma.2015 <- plan.data.2015 %>%
  inner_join(service.data.2015 %>% select(contractid, fips),
             by = c("contractid","fips")) %>%
  filter(!state %in% c("VI","PR","MP","GU","AS",""),
         snp == "No",
         (planid < 800 | planid >= 900),
         !is.na(planid), !is.na(fips)) %>%
  left_join(pen.2015 %>% ungroup() %>% rename(state_long = state, county_long = county) %>%
...
full.2015 <- ma.2015 %>%
  ...
  left_join(rebate.2015 %>% select(-contract_name, -plan_type), by = c("contractid","planid")) %>%
  mutate(
...
# Save data ---------------------------------------------------------------
write_csv(full.2015,"data/output/data-2015.csv")
