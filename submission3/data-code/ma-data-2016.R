...
plan.data.2016 <- as_tibble(plan.year)
...
service.data.2016 <- service.year %>%
...
# Landscape data ----------------------------------------------------------
# Import data
ma.path.a <- paste0("data/input/ma/landscape/Extracted Data/2016LandscapeSource file MA_AtoM 04222016.csv")
...
ma.path.b <- paste0("data/input/ma/landscape/Extracted Data/2016LandscapeSource file MA_NtoW 04222016.csv")
...
mapd.path.a <- paste0("data/input/ma/landscape/Extracted Data/PartCD/2016/Medicare Part D 2016 Plan Report 04222016.xls")
...
mapd.path.b <- paste0("data/input/ma/landscape/Extracted Data/PartCD/2016/Medicare Part D 2016 Plan Report 04222016.xls")
...
landscape.2016 <- mapd.clean.merge(ma.data=ma.data, mapd.data=mapd.data, y)
...
# Risk rebate data --------------------------------------------------------
# Import data
ma.path.a <- "data/input/ma/cms-payment/2016/2016PartCPlanLevel.xlsx"
risk.rebate.a <- read_xlsx(ma.path.a,range="A4:G2777",
                           col_names=c("contractid","planid","contract_name","plan_type",
                                       "riskscore_partc","payment_partc","rebate_partc"))
ma.path.b <- "data/input/ma/cms-payment/2016/2016PartDPlans.xlsx"
risk.rebate.b <- read_xlsx(ma.path.b,range="A4:H3707",
                           col_names=c("contractid","planid","contract_name","plan_type",
...
# Merge data --------------------------------------------------------------
ma.2016 <- plan.data.2016 %>%
  inner_join(service.data.2016 %>% select(contractid, fips),
             by = c("contractid","fips")) %>%
  filter(!state %in% c("VI","PR","MP","GU","AS",""),
         snp == "No",
         (planid < 800 | planid >= 900),
         !is.na(planid), !is.na(fips)) %>%
  left_join(pen.2016 %>% ungroup() %>% rename(state_long = state, county_long = county) %>%
...
ma.2016 <- ma.2016 %>%
  mutate(year = coalesce(as.numeric(year.x), as.numeric(year.y), 2016)) %>%
  select(-starts_with("year."))

full.2016 <- ma.2016 %>%
  left_join(state.2016, by = "state") %>%
  left_join(landscape.2016 %>% mutate(state=str_to_lower(state)), by = c("contractid","planid","state_name" = "state","county")) %>%
  left_join(rebate.2016 %>% select(-contract_name, -plan_type), by = c("contractid","planid")) %>%
  mutate(
  ...
  left_join(ffscosts.2016 %>% select(-state) %>% filter(!is.na(ssa)), by = c("ssa","year")) %>%
  mutate(
...
# Save data ---------------------------------------------------------------
write_csv(full.2016,"data/output/data-2016.csv")
